...
function DocumentRow({ file }) {
  return (
    <button className="doc-row" type="button" onClick={(event) => runLegacyHandler(event, `openMockDownload('${file}','${file}')`)}>
      <div className="doc-icon pdf">PDF</div>
      <div className="doc-info"><div className="doc-title">{file.replaceAll("_", " ")}</div><div className="doc-meta">Document de gouvernance</div></div>
    ...
    <div className={`km-navbar governance-tabbar ${className}`.trim()}>
      {items.map((item, index) => (
        <React.Fragment key={item.id}>
          ...
          <button
            type="button"
            data-governance-tab={item.id}
            className={`km-nav-item${activeId === item.id ? " active" : ""}`}
...
export default function GouvernanceSection() {
  ...
  const documents = (committee.documents || []).filter((file) => file.toLowerCase().includes(documentQuery.toLowerCase()));
  ...
  useEffect(() => { window.lucide?.createIcons(); }, [activeTab, systemTab, selectedCommittee, documentQuery]);
  ...
  return (
    <div id="view-gouvernance" className="view-section km-container">
      ...
      {activeTab === "systeme" ? <div><p className="section-intro">Le système de gouvernance regroupe les instances décisionnelles, les comités spécialisés et leurs documents de référence.</p><GovernanceNav className="governance-system-tabs" activeId={systemTab} onSelect={setSystemTab} items={[{ id: "conseil", label: "Conseil d'administration" }, { id: "comites", label: "Comités spécialisés" }, { id: "documents", label: "Espace documentaire" }]} />{systemTab === "conseil" ? <div className="content-card governance-system-panel"><h3>Conseil d'administration</h3><p className="section-intro">Instance centrale d'orientation, de décision et de supervision de la CMR.</p><BoardPage board={board} /></div> : null}{systemTab === "comites" ? <div className="content-card governance-system-panel"><h3>Comités spécialisés</h3><CommitteeFilters committees={committees} selected={selectedCommittee} onSelect={setSelectedCommittee} /><div className="governance-committee-detail"><h4>{committee.name}</h4><p>{committee.description}</p><div className="governance-detail-grid"><div><h5>Missions</h5><ul>{(committee.missions || []).map((item) => <li key={item}>{item}</li>)}</ul></div><div><h5>Membres</h5><ul>{(committee.members || []).map((item) => <li key={item}>{item}</li>)}</ul></div></div><DocumentRow file={committee.charter} /></div></div> : null}{systemTab === "documents" ? <div className="content-card governance-system-panel"><div className="governance-access-note"><i data-lucide="lock-keyhole" /><div><strong>Accès restreint</strong><span>Les documents sont affichés selon les habilitations du profil connecté.</span></div></div><CommitteeFilters committees={committees} selected={selectedCommittee} onSelect={setSelectedCommittee} /><div className="section-search-row"><i data-lucide="search" /><input value={documentQuery} onChange={(event) => setDocumentQuery(event.target.value)} placeholder="Rechercher un document du comité..." /></div><div className="governance-document-list">{documents.map((file) => <DocumentRow file={file} key={file} />)}{!documents.length ? <p className="empty-state">Aucun document trouvé.</p> : null}</div></div> : null}</div> : null}
